...
async function resolveLatestK8sVersion(cluster: {
  ...
  if (!secretKey || !region) return FALLBACK_K8S_VERSION;
  try {
    const res = await fetchWithTimeout(
      `https://api.scaleway.com/k8s/v1/regions/${encodeURIComponent(region)}/versions`,
      { headers: { "X-Auth-Token": secretKey } }
    );
    if (!res.ok) return FALLBACK_K8S_VERSION;
    const body = (await res.json()) as { versions?: Array<{ name: string }> };
    const names = (body.versions ?? []).map((v) => v.name).filter(Boolean);
    if (names.length === 0) return FALLBACK_K8S_VERSION;
