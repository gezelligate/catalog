...
async function hcloudJson(token, segment, init) {
    ...
    if (!res.ok)
        throw new Error(`Hetzner API ${segment} returned ${res.status}`);
    if (res.status === 204)
        return null;
    return (await res.json());
}
function lbNameFromBaseDomain(baseDomain) {
    if (!baseDomain)
        return "gezelligate-traefik";
...
async function ensureSshKey(rootDir, ctx) {
    ...
    try {
        ...
        await ctx.run("ssh-keygen", ["-t", "ed25519", "-N", "", "-C", "gezelligate", "-f", privateKeyPath], rootDir, process.env);
    }
    await fs.chmod(privateKeyPath, 0o600);
    const publicKey = (await fs.readFile(publicKeyPath, "utf8")).trim();
    return { privateKeyPath, publicKeyPath, publicKey };
}
async function ensureK3sToken(rootDir) {
...
async function validateServerTypes(cluster) {
    ...
    if (!stRes.ok || !dcRes.ok) {
        throw new Error(`Hetzner API rejected the token (server_types=${stRes.status}, datacenters=${dcRes.status}). ` +
            `Re-check your API token in the Cloud Provider step.`);
    }
    const { server_types } = (await stRes.json());
    const { datacenters } = (await dcRes.json());
    ...
    const orderableNames = new Set(server_types.filter((t) => orderableIds.has(t.id)).map((t) => t.name));
    const wanted = [cluster.controlPlaneType, cluster.nodePoolType].filter((n) => typeof n === "string");
    const missing = wanted.filter((n) => !orderableNames.has(n));
    if (missing.length > 0) {
        const sorted = Array.from(orderableNames).sort();
        throw new Error(`Server type(s) not orderable in location "${cluster.geography}": ${missing.join(", ")}.\n` +
...
async function adoptOrphans(tofuDir, token, ctx) {
    const env = ctx.env;
    ...
    if (!tracked.has("hcloud_network.gezelligate")) {
        const nets = await hcloudGet(token, "/networks?name=gezelligate");
    ...
    for (let i = 0; i < 20; i += 1) {
        ...
        if (tracked.has(key))
            continue;
        const w = await hcloudGet(token, `/servers?name=${encodeURIComponent(name)}`);
        if (w.servers.length === 0)
            break;
...
async function installCloudControllerManager(ctx) {
    ...
    const kenv = { ...ctx.env, KUBECONFIG: ctx.kubeconfigPath };
...
async function ensureNodesInitialized(ctx) {
    const kenv = { ...ctx.env, KUBECONFIG: ctx.kubeconfigPath };
    ...
    for (const node of tainted) {
        ...
        await ctx.run("kubectl", ["taint", "nodes", node, `${taintKey}:NoSchedule-`], ctx.rootDir, kenv);
    }
}
async function reconcileLbTargets(ctx) {
    ...
    for (const lb of load_balancers) {
        ...
        if (lb.targets.length > 0)
            continue;
        console.log(`==> LB ${lb.name} has 0 targets; attaching control plane (server_id=${cpServer.id}) via API`);
...
export const lifecycle = {
    ...
    async listResources(cluster) {
        ...
        const networks = (await networksRes.json()).networks ??
            [];
        const lbs = (await lbsRes.json()).load_balancers ?? [];
        const keys = (await keysRes.json()).ssh_keys ?? [];
        const serverTypes = (await stRes.json()).server_types ?? [];
        const stPriceMap = new Map();
        for (const st of serverTypes) {
            const inner = new Map();
            for (const p of st.prices ?? [])
                inner.set(p.location, Number(p.price_monthly.gross));
    ...
    async captureKubeconfig(ctx) {
        ...
        const cpIp = (await ctx.capture("tofu", ["output", "-raw", "control_plane_ip"], ctx.tofuDir, ctx.env)).trim();
        const sshKey = ctx.env.TF_VAR_ssh_private_key_path;
    ...
    async afterTofuDestroy(ctx) {
        ...
        try {
            ...
            console.error(`warning: hccm LB cleanup failed: ${err instanceof Error ? err.message : err}`);
            console.error("Check your Hetzner dashboard and delete any remaining LBs manually.");
        }
    },
    async collectMetadata(ctx) {
        try {
            const controlPlaneIp = (await ctx.capture("tofu", ["output", "-raw", "control_plane_ip"], ctx.tofuDir, ctx.env)).trim();
